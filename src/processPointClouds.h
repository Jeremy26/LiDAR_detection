...
struct KdTree
{
	...
	void searchHelper(pcl::PointXYZI target, Node*node, uint depth, float distanceTol, std::vector<int>& ids)
	{
		...
		{
			// LOOK AT THE ROOT
			// EUCLIDIEN DISTANCE BETWEEN ROOT AND TARGET
			// IF WITHIN DISTANCE; ADD IT AND MOVE TO THE NEXT POINT
			...
			// ELSE SPLIT X AND CHECK; FORGET POINTS IN THE WRONG REGION
            if(depth%2 == 0)
            {
                if (target.x - distanceTol < node->point.x) searchHelper(target,node->left,depth+1,distanceTol, ids);
                if (target.x + distanceTol > node->point.x) searchHelper(target,node->right,depth+1,distanceTol, ids);
            }
            else
            {
                if (target.y - distanceTol < node->point.y) searchHelper(target,node->left,depth+1,distanceTol, ids);
                if (target.y + distanceTol > node->point.y) searchHelper(target,node->right,depth+1,distanceTol, ids);
            }
